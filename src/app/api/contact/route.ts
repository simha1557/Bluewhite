...
function generateEmailTemplate({ name, email, company, message, contact, clientIP }: {
  name: string
  email: string
  company?: string
  message: string
      ...
      <div style="background: white; padding: 20px; border-radius: 0 0 8px 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <div style="margin: 20px 0;">
          ...
          <table style="width: 100%; border-collapse: collapse; background: #f8f9fa; border-radius: 8px; overflow: hidden;">
            ...
            ${contact?.id ? `
            <tr>
              <td style="padding: 12px 15px; font-weight: bold; color: #666;">Database ID:</td>
              <td style="padding: 12px 15px; color: #333;">${contact.id}</td>
            </tr>
            ` : ''}
          </table>
        </div>
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        const emailResult = await resend.emails.send({
          ...
          html: generateEmailTemplate({ name, email, company, message, contact, clientIP }),
          text: `
...
${contact?.id ? `Database ID: ${contact.id}` : ''}
